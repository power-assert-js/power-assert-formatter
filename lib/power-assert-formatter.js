    ...
    PowerAssertContextRenderer.prototype.renderBinaryExpressionDiff = function (lines) {
        ...
        pairs.filter(function (pair) {
            ...
            var delta = stringDiff.diff(pair.left.value, pair.right.value),
                totalDiff = {right: [], left: []},
                colSize;
            lines.push('');
            that.lineDiffs(delta).forEach(function (diff) {
                totalDiff.left.push(diff.left);
                totalDiff.right.push(diff.right);
            });
            colSize = Math.max(that.widthOf(pair.left.name), that.widthOf(pair.right.name));
            lines.push(that.label(pair.left.name, colSize) + ': ' + totalDiff.left.join('...'));
            lines.push(that.label(pair.right.name, colSize) + ': ' + totalDiff.right.join('...'));
        });
    };

    PowerAssertContextRenderer.prototype.label = function (str, size) {
        return this.fillColumns(createRow(size, ' '), str).join('');
    };

    PowerAssertContextRenderer.prototype.fillColumns = function (row, str) {
        var i, width = this.widthOf(str);
        for (i = 0; i < width; i += 1) {
            row.splice(i, 1, str.charAt(i));
        }
        return row;
    };

    PowerAssertContextRenderer.prototype.lineDiffs = function (delta) {
        var lines = parseTextDiff(delta[0]),
            that = this;
        return lines.map(function (line) {
            var diff = {left: '', right: ''};
            line.pieces.forEach(function (piece) {
                var pieceText = decodeURIComponent(piece.text),
                    tmpStr = '(' + pieceText + ')',
                    //tmpStr = pieceText,
                    spacer = createRow(that.widthOf(tmpStr), ' ').join('');
                    //spacer = '';
                switch (piece.type) {
                case 'context':
                    diff.left += pieceText;
                    diff.right += pieceText;
                    break;
                case 'added':
                    diff.left += spacer;
                    diff.right += tmpStr;
