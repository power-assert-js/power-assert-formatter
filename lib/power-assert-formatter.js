...
(function (root, factory) {
    ...
    PowerAssertContextRenderer.prototype.renderLines = function () {
        ...
        this.collectBinaryExpression(this.context).forEach(function (pair) {
            var patch;
    ...
    PowerAssertContextRenderer.prototype.collectBinaryExpression = function (context) {
        var pairs = [], that = this;
        context.args.forEach(function (arg) {
            ...
            var espathList = arg.events.map(function (ev) {
                ...
                espathToPair = {},
                jsAST = esprima.parse(context.source.content, {tolerant: true, loc: true, raw: true});
            estraverse.traverse(extractExpressionFrom(jsAST), {
                enter: function (currentNode, parentNode) {
                    ...
                    if (that.isTargetBinaryExpression(parentNode) && currentNode.type === syntax.Literal && typeof espathToPair[parentEspath] === 'object') {
                        espathToPair[parentEspath][currentProp] = {name: '' + currentProp + '(' + currentNode.type + ')', value: currentNode.value};
                    }
                    ...
                    if (that.isTargetBinaryExpression(currentNode)) {
                        pair = {};
                        espathToPair[espath] = pair;
                        pairs.push(pair);
                    }
                    if (that.isTargetBinaryExpression(parentNode)) {
                        if (currentNode.type === syntax.Identifier) {
                            espathToPair[parentEspath][currentProp] = {name: currentNode.name, value: espathToValue[espath]};
                        } else {
                            espathToPair[parentEspath][currentProp] = {name: '' + currentProp + '(' + currentNode.type + ')', value: espathToValue[espath]};
                        }
                    }
                }
            });
