...
(function (root, factory) {
    ...
    PowerAssertContextRenderer.prototype.renderLines = function () {
        ...
        pairs.forEach(function (pair) {
            if (isBothType(pair, 'string')) {
                that.showStringDiff(pair, lines);
            } else if (isComparable(pair)) {
                that.showExpectedAndActual(pair, lines);
            }
        });
    ...
    PowerAssertContextRenderer.prototype.showExpectedAndActual = function (pair, lines) {
        lines.push('');
        lines.push('$$$ [' + typeof pair.right.value + '] ' + pair.right.code);
        lines.push('### [' + typeof pair.left.value  + '] ' + pair.left.code);
        lines.push('$ ' + JSON.stringify(pair.right.value));
        lines.push('# ' + JSON.stringify(pair.left.value));
    };

    PowerAssertContextRenderer.prototype.showStringDiff = function (pair, lines) {
        var patch;
    ...
    function isBothType(pair, type) {
        return typeof pair.left.value === type && typeof pair.right.value === type;
    }
            ...
            onEachEsNode(arg, context.source.content, function (esNode) {
                ...
                if (!esNode.isCaptured()) {
                    if (isTargetBinaryExpression(esNode.parentNode) && esNode.currentNode.type === syntax.Literal) {
                        if (esNode.getParentEsNode().isCaptured()) {
