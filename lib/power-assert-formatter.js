...
(function (root, factory) {
    ...
    PowerAssertContextRenderer.prototype.renderBinaryExpressionDiff = function (lines) {
        ...
        pairs.filter(function (pair) {
            ...
            var delta = stringDiff.diff(pair.left.value, pair.right.value),
                ...
                colSize;
            lines.push('');
            that.chunks(delta).forEach(function (chunk) {
                totalDiff.left.push(chunk.left);
                totalDiff.right.push(chunk.right);
            });
    ...
    PowerAssertContextRenderer.prototype.chunks = function (delta) {
        ...
        return lines.map(function (line) {
            ...
            line.pieces.forEach(function (piece) {
                var pieceText = decodeURIComponent(piece.text),
                    diffStr = '(' + pieceText + ')',
                    //diffStr = pieceText,
                    //spacer = createRow(that.widthOf(diffStr), ' ').join('');
                    spacer = '';
                switch (piece.type) {
                case 'context':
                    diff.left += pieceText;
